...
def _split_domain_and_name(name: str) -> tuple[str, str]:
    """
    Split a name of the form "domain::name" into (domain, name).
    If "::" is not present, return ("", name).
    """
    parts = name.split("::", 1)
    if len(parts) == 2:
...
def write_zarr(pde_dataset: PdeDataSet, data_functions: DataFunctions, mesh: CartesianMesh, zarr_dir: Path) -> None:
    ...
    for t in range(num_t):
        ...
        for v in volume_data_vars:
            ...
            z1[t, c, :, :, :] = var_data
            domain_name, var_name = _split_domain_and_name(v.var_info.var_name)
        ...
        for f in volume_functions:
            ...
            z1[t, c, :, :, :] = func_data
            domain_name, function_name = _split_domain_and_name(f.name)
            if t == 0:
                channel_metadata.append({
                    "index": c,
                    "label": function_name,
