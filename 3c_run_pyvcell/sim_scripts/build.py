...
def build_tensed_model(relaxed_vcml_file, application="Spatial"):
    ...
    # void.analytic_expr = '(((x >= 0.0) && (x < L_kin_x1) && (y >= 1.55) && (y <= 1.85)) || ((x > R_kin_x2) && (x <= 1.3) && (y >= 1.55) && (y <= 1.85)))'
    void.analytic_expr = '0.0' # just remove void and update KT compartments to match new geometry
    plot_2d_geo(relaxed_model.applications[0].geometry, title = "Relaxed Geometry")

    plot_2d_geo(geo, title = "Tensed Geometry")
    compare_models(relaxed_model, tensed_model, name1="Relaxed", name2="Tensed", verbose=1) # check model differences
...
def build_transition_model(relaxed_vcml_file, field_data_dir, application="Spatial"):
    ...
    tensed_model = build_tensed_model(relaxed_vcml_file, application) # start with tensed model as the base for the transition model, since we want to keep the updated geometry and parameters
    ...
    model.add_model_parameter("max_vel", '0.2875/delT') #KT distance from relaxed to tensed

    # advection
    model.add_model_parameter("sigma_y", 0.15)
    model.add_model_parameter("sigma_x", 0.2875)
    model.add_model_parameter("norm_x", 'exp(( - ((x_L_L - x_mid) ^ 2.0) / (2.0 * (sigma_x_X ^ 2.0))))')
