...
for idx, case in enumerate(alpha_cases):
    ...
    ax1.plot(phi_range, psi_alpha, 'b-', linewidth=2.5, label=f'ψ(φ, α={alpha:+.1f})')
    
    # Plot the equilibrium points
    psi_m = energy_with_alpha(phi_m, alpha)
    psi_p = energy_with_alpha(phi_p, alpha)
    ax1.plot([phi_m, phi_p], [psi_m, psi_p], 'ro', markersize=8, zorder=3, label='Equilibria')
    ...
    ax1.plot(phi_range, tangent, 'r--', linewidth=2, alpha=0.7, label='Common tangent')
    ...
    ax1.set_title(f'α = {alpha:+.1f}\n(Common Tangent)', fontsize=11, fontweight='bold')
    ...
    ax2.plot([phi_m, phi_p], [psi_2_m, psi_2_p], 'ro', markersize=8, zorder=3, label='Equilibria')
    ...
    ax2.set_title(f'α = {alpha:+.1f}\n(With A Parameter)', fontsize=11, fontweight='bold')
    ...
    print(f"Energy with α at equilibria: ψ({phi_m:.4f}) = {psi_m:.6f}, ψ({phi_p:.4f}) = {psi_p:.6f}")
    print(f"Modified energy ψ₂ at equilibria: ψ₂({phi_m:.4f}) = {psi_2_m:.6f}, ψ₂({phi_p:.4f}) = {psi_2_p:.6f}")
    print(f"Common tangent slope: {(psi_p - psi_m) / (phi_p - phi_m):.4f}")

plt.tight_layout()
plt.savefig('/Users/smgroves/Documents/GitHub/VCell_Analysis/5_vcell_to_ch/spinodal_point_plot/energy_progression.png', dpi=300, bbox_inches='tight')
plt.show()
