...
def plot_hist_sizes(indir,
                    celltype,
                    threshold=10000,
                    save=False,
                    filter_formfactor=True,
                    suffix="",
                    sort_experiment=False):
    print(celltype)
    df = pd.read_csv(f"{indir}/{celltype}_Cells.csv", index_col=None, header=0)
    if sort_experiment:
        experiments = list(set(df["Metadata_ExperimentName"].values))
    else:
        experiments = [celltype]
    ...
    for experiment in experiments:
        if experiment == celltype:
            tmp_df = df
            tmp_subset = subset
        else:
            tmp_df = df.loc[df['Metadata_ExperimentName'] == experiment]
            tmp_subset = subset.loc[df['Metadata_ExperimentName'] == experiment]

        (mu, sigma) = norm.fit(tmp_subset['Volume_um'])
        print(mu)
        # the histogram of the data

        n, bins, patches = plt.hist(tmp_df['Volume_um'],
                                    60,
                                    alpha=0.75,
                                    density=True)
        plt.close()
        ...
        plt.title(f"{experiment} Cell Sizes")
        plt.ylabel("Density")
        ...
        plt.text(
            ...
            f'\n threshold = {threshold}'
            f'\n Filter Form Factor = {filter_formfactor}',
            ha='right',
            va='top',
        ...
        if save:
            plt.savefig(
                f"{indir}/{experiment}_threshold{threshold}_size_histogram{suffix}.pdf"
            )
            plt.close()
        else:
            plt.show()
