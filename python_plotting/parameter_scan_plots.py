...
def lineplot(species, in_dir, sim_prefix, name_scan, num_scans, xmax = None, xmin = 0, log = False,
    ...
    if name is not None:
        ...
        plt.title(f"{name} Concentration in {location} over {plot_data['Time'].max()} seconds")
        ...
        if column.startswith('Sum'):
            ...
            plt.title(f"{species} Concentration in {location} over {plot_data['Time'].max()} seconds")
    ...
    if highlight is not None:
        ...
        sns.lineplot(x = tmp['Time'].to_numpy(), y= tmp[column].to_numpy(), color = 'orange',linestyle = "dotted", ax = ax, legend=False)
    ...
    plt.tight_layout()
    plt.savefig(f"./figures/scan-{name_scan}_species-{species}_loc-{location}{suffix}.png")
    # plt.show()
    plt.close()
#
...
def plot_across_models(species, plot_list, in_dir, location = 'ic',column = "Sum_Active", active = 'active',
    ...
    plt.savefig(f"./figures/{name_plot}-{species}_loc-{location}.png")
    # plt.show()
    plt.close()
...
# plot_across_models('CPC', plot_list, in_dir, location='ic',name_plot="Haspin_width",active= 'all')
# plot_across_models('CPC', plot_list, in_dir, location='kt',name_plot="Haspin_width",active= 'all')
#
