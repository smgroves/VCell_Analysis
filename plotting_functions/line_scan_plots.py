...
def line_scan_plot(
    ...
    for i, timepoint in enumerate(t_list):
        ...
        for s in sim.keys():
            ...
            for f in os.listdir(f"{in_dir}/{s}"):
                if species in f and timepoint in f:
                    tmp = pd.read_csv(
                        f"{in_dir}/{s}/{f}", skiprows=10, header=None, index_col=None
                    )
        ...
        if show_auc:
            ...
            for hue in sorted(list(set(long_horiz_df["Simulation"]))):
                ...
                tmp = tmp.loc[(tmp["x"] > 0.425) & (tmp["x"] < 1.175)]
                aucs[f"{hue}"] = auc(tmp["x"], tmp["value"])
                all_aucs[f"{hue}"].append(auc(tmp["x"], tmp["value"]))

            # create tuples of positions
            positions = [(0.94 - 0.05 * x) for x in range(2)]
            # add text
            plt.text(
                0.01,
                0.99,
                "Area under curve between relaxed KT",
                ha="left",
                va="top",
            ...
            plt.axvline(0.425, linestyle="--", c="gray", alpha=0.5)
            plt.axvline(1.175, linestyle="--", c="gray", alpha=0.5)
            ...
            plt.fill_between(
                x=horiz_df["x"],
                y1=y1,
                where=(0.425 < horiz_df["x"]) & (horiz_df["x"] < 1.175),
                color="lightgrey",
                alpha=0.2,
            )
        plt.tight_layout()
        ...
        if save:
            if os.path.isdir(f"{out_dir}/line_scans/{name}/horiz"):
                pass
            else:
                os.makedirs(f"{out_dir}/line_scans/{name}/horiz")
            plt.savefig(
                f"{out_dir}/line_scans/{name}/horiz/{species}_t{timepoints[i]}_horiz.{savetype}"
            )
            plt.close()
        else:
            plt.show()
        ...
        plt.xlabel(
            f"Total {species} across vertical axis \n at Time {timepoints[i]}s (uM)"
        )
        plt.ylabel("Y dimension (um)")
        plt.title(
            f"Line Scan across Vertical Midpoint Axis \n at Time {timepoints[i]}s"
        )
        plt.xlim(0, ymax)
        plt.tight_layout()
        if save:
            if os.path.isdir(f"{out_dir}/line_scans/{name}/vert"):
                pass
            else:
                os.mkdir(f"{out_dir}/line_scans/{name}/vert")
            plt.savefig(
                f"{out_dir}/line_scans/{name}/vert/{species}_t{timepoints[i]}_vert.{savetype}"
            )
            plt.close()
        else:
            plt.show()
    ...
    if show_auc:
        ...
        plt.ylabel("AUC")
        plt.title(
            "Area under curve of horizontal line scan \n between relaxed KTs across simulations"
        )
        if save:
            plt.savefig(
                f"{out_dir}/line_scans/{name}/horiz/AUC_{species}_horiz.{savetype}"
            )
            plt.close()
        else:
            plt.show()
...
sim = {
    ...
    "SimID_278800752_2__exported": "11_06_24_relaxed_RefModel_MonseData_Sgo1_down_0.013",
    "SimID_278800752_3__exported": "11_06_24_relaxed_RefModel_MonseData_Sgo1_down_0.019",
    "SimID_278800752_4__exported": "11_06_24_relaxed_RefModel_MonseData_Sgo1_down_0.026 REFERENCE",
    "SimID_278878127_1__exported": "11_06_24_relaxed_RefModel_MonseData_Sgo1_up_0.044",
    "SimID_278878127_2__exported": "11_06_24_relaxed_RefModel_MonseData_Sgo1_up_0.063",
    "SimID_278878127_3__exported": "11_06_24_relaxed_RefModel_MonseData_Sgo1_up_0.081",
    "SimID_278878127_4__exported": "11_06_24_relaxed_RefModel_MonseData_Sgo1_up_1",
}
